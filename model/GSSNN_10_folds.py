...
parser = argparse.ArgumentParser(description='GSSNN_10_folds.')
...
def cross_folds(split):
    ...
    if split == 10:
        ...
        r_sp = split * fold
    l_sp = r_sp - fold
    test_mask = torch.zeros(len(dataset), dtype=torch.long)
    test_mask[l_sp:r_sp] = 1

    test_dataset = dataset[test_mask.eq(1)]
    train_dataset = dataset[test_mask.eq(0)]
...
for i in range(1, 11):
    ...
    train_dataset, test_dataset = cross_folds(i)
    test_loader = DataLoader(test_dataset, batch_size=batch_size)
    train_loader = DataLoader(train_dataset, batch_size=batch_size)

    high_acc = 0
    for epoch in range(1, args.epoch):
        train_loss = train(epoch, train_loader, train_dataset)
        train_acc = test(train_loader)
        test_acc = test(test_loader)
        if test_acc >= high_acc:
            high_acc = test_acc
    acc[i - 1] = high_acc
    print('Fold: {:02d}, Highest_acc: {:.4f}'.format(i, high_acc))

print('Mean: {:.4f}, Std: {:.4f}'.format(acc.mean(), acc.std()))
print(args)
